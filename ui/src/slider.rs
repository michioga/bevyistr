...
impl SliderState {
    pub fn normalized(&self) -> f32 {
        if (self.max - self.min).abs() < 1.0e-9 { return 0.0; }
...
pub fn spawn_slider(parent: &mut ChildSpawnerCommands, config: SliderConfig) -> Entity {
    ...
    let initial_t = if (config.max - config.min).abs() < 1e-9 { 0.0f32 }
                    else { ((config.value - config.min) / (config.max - config.min)).clamp(0.0, 1.0) };
    ...
    .with_children(|container| {
        ...
        container.spawn((
            ...
            SliderThumb { track: track_entity },
...
pub(crate) fn update_sliders(
    ...
    mut track_query: Query<(Entity, &mut SliderState, &ComputedNode, &UiGlobalTransform), With<SliderTrack>>,
        ...
        if let Some(cursor) = window.cursor_position() {
            ...
            if buttons.just_pressed(MouseButton::Left) {
                for (_, mut state, track_node, track_gt) in &mut track_query {
                    ...
                    if cursor.x >= origin.x && cursor.x <= origin.x + size.x
    ...
    for (track_entity, state, track_node, _track_gt) in &mut track_query {
        ...
        for (thumb, mut node) in &mut thumb_query {
            if thumb.track == track_entity { node.left = px(t * usable_w); }
        }
        for (fill, mut node) in &mut fill_query {
            if fill.0 == id { node.width = px(t * usable_w); }
        }
