...
グラフの値域は固定対象の全履歴から求めます。モデル全体のコンター色範囲や変形倍率とは独立しています。表示はモデルの単位系に従い、単位を推測・変換しません。グラフ自体は閲覧専用で、フレーム移動には既存のスライダーやコマ送りボタンを使います。複数対象の重ね描き、拡大操作は未対応です。
...
3. 履歴直下の**Export history CSV...**を押して保存先を選びます。
...
| `time_or_load_factor`, `time_status` | 格納されたTime／荷重係数。有限なら`recorded_or_default`、非有限なら空欄と`unavailable`。読込処理が未記録時刻を0として扱うため、0が実時刻か既定値かは区別できません。秒や固有振動数とは断定しません。 |
