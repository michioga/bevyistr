...
![熱解析の温度コンター、固定プローブと2点比較](../assets/images/results-heat-probe.png)

`16_heat_block`のPVTUを直接開いた例です。Node 92の温度20とNode 5845の温度100を比較し、
Node 5845をPINNEDにしています。単位はモデルの単位系に従います。
HISTORYの`2/2 samples`は「1フレーム×2対象」で、2フレームあるという意味ではありません。
時系列の線ではなく同じFrame 1上の2点となり、黄色の縦線は現在フレームを示します。
変位がないためDeformationは利用できません。
この状態でCSVを保存すると、比較一覧の2点ではなくPINNEDのNode 5845だけを1行に保存します。
